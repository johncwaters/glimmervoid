...
function buildMemoryDistillPrompt({
  ...
  return [
    ...
    '- A DISTILLED result must contain at least one claim. An empty claim set refuses this whole run.',
    '- Every claim must cite at least one record id copied exactly from the markers above. ONE claim without one refuses this whole run.',
    `- At most ${MAX_CLAIM_IDS} record ids per claim. ONE claim over it refuses this whole run, so split a broadly supported fact into several claims or cite its strongest ${MAX_CLAIM_IDS} records.`,
    ...
    '- Every claim must carry non-empty text. ONE claim without text refuses this whole run.',
    '- Never put a high-entropy token in claim text. ONE claim carrying one refuses this whole run.',
    '- Every record a claim cites must be of the claim\'s own kind. ONE claim mixing kinds refuses this whole run.',
...
function buildIncrementalDistillPrompt({
  ...
  return [
    ...
    '- Retiring every standing claim is allowed only when your operations also add one. Leaving no claim at all while records still exist refuses this whole run, so pair a retire that empties the set with its replacement claim.',
    ...
    `- At most ${maxClaims} operations may be output. ONE operation over it refuses this whole run.`,
    '- Every claim must cite at least one record id copied exactly from the observations above. ONE claim without one refuses this whole run.',
    `- At most ${MAX_CLAIM_IDS} record ids per claim. ONE claim over it refuses this whole run, so split a broadly supported fact into several claims or cite its strongest ${MAX_CLAIM_IDS} records.`,
    `- The ${maxClaimChars} character limit is HARD and counted per claim: ONE claim over it refuses this whole run, so split a long fact into two claims rather than writing one long one.`,
    '- Every claim must carry non-empty text. ONE claim without text refuses this whole run.',
    '- Never put a high-entropy token in claim text. ONE claim carrying one refuses this whole run.',
