...
import type { CanonWatermark, MemoryRecord, ProjectionManifest } from './core/memory-core.ts';
import * as distillCore from './core/memory-distill-core.ts';
import type { DistillClaim, HandledClaim, MergedClaimsResult } from './core/memory-distill-core.ts';
import {
...
interface MemoryDistillSpawnOptions {
...
interface MemoryDistillerOptions {
  ...
  config?: distillCore.DistillConfig;
  spawnDistill?: SpawnDistill;
  readResult?: (resultPath: string) => Promise<Record<string, unknown> | null>;
...
async function readDistillResultFile(resultPath: string): Promise<Record<string, unknown> | null> {
  try {
...
function makeMemoryDistillWorkDir(): Promise<string> {
...
function createMemoryDistiller(deps: MemoryDistillerOptions = {}) {
  ...
  function spawnWithTimeout(
    { prompt, cwd, resultPath, onPending }: {
      prompt: string;
      cwd: string;
      resultPath: string;
      onPending: (promise: Promise<unknown>) => void;
    },
  ): Promise<SpawnOutcome> {
    return raceWithAbort<SpawnOutcome>({
      ...
      start: (signal) => Promise.resolve(spawnDistill({
        ...
        .catch((error: unknown) => ({ timedOut: false, parsed: null, error: firstLine(errorMessage(error)) })),
    });
  }

  function filesFor(claims: HandledClaim[], valid: MemoryRecord[]) {
  ...
  async function spawnForResult(
    { prompt }: { prompt: (resultPath: string) => string },
  ): Promise<{ error: string | null; parsed: Record<string, unknown> | null }> {
    let workDir: string | null = null;
    try {
      workDir = await makeWorkDir();
    } catch (error) {
      return { error: `no work dir: ${firstLine(errorMessage(error))}`, parsed: null };
    }
    ...
    try {
      ...
      return { error: `no prompt file: ${firstLine(errorMessage(error))}`, parsed: null };
    }
  ...
  async function publishMerged({
    ...
    if (!verified.ok) {
      await noteOutcome(memoryStore, { advanced: false, cursor, failures });
      return report({
        status: 'error', reason: `published but ${verified.reason}`, version: published.version ?? null, mode,
      });
    }
    await noteOutcome(memoryStore, { advanced: advances, cursor, failures });
    log.note(`distilled ${built.claimCount} claim(s) from ${built.recordCount} record(s) in ${mode} mode: ${published.published ? 'published' : 'unchanged'} ${String(published.version).slice(0, 12)}`);
    return report({
      status: 'published',
      verdict,
  ...
  async function compact({
    ...
    const spawned = await spawnForResult({
      prompt: (resultPath: string) => distillCore.buildMemoryDistillPrompt({
    ...
    if (checked.verdict !== 'DISTILLED') return report({ status: 'current', verdict: checked.verdict ?? null, mode: 'full' });
    const stray = checked.claims.filter((claim: DistillClaim) => (claim.project || null) !== project);
    if (stray.length > 0) {
      await noteOutcome(memoryStore, { advanced: false, cursor: 0, failures });
  ...
  async function distillDelta({
    ...
    const spawned = await spawnForResult({
      prompt: (resultPath: string) => distillCore.buildIncrementalDistillPrompt({
...
export type { DistillReport, MemoryDistillerOptions, SpawnDistill };
