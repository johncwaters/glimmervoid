...
test('declaredActiveCount filters out-of-band idled ids; an id-less entry always counts', () => {
  ...
  assert.equal(declaredActiveCount(entries, new Set<string>()), 3);
...
test('the default shell task TTL covers a forty-minute external agent run', () => {
  ...
  assert.equal(declaredActiveCount(entries, new Set<string>(), 40 * 60 * 1000), 1);
...
test('declaredActiveCount: idleNameCount subtracts from surviving teammate-type entries', () => {
  ...
  assert.equal(declaredActiveCount(entries, new Set<string>(), 0, undefined, 0), 3, 'no idle names, nothing subtracted');
  assert.equal(declaredActiveCount(entries, new Set<string>(), 0, undefined, 1), 2, 'one idle name drains one teammate');
  assert.equal(declaredActiveCount(entries, new Set<string>(), 0, undefined, 3), 0, 'all three idle names drain all three teammates');
});
