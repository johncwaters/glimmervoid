...
function pack(overrides: Partial<MillPack> = {}): MillPack {
  return {
    name: 'house-rules',
    specValid: true,
    specErrors: [],
...
function builtPack(buildOverrides: Partial<MillBuild> = {}): BuiltPack {
  return { ...pack(), built: build(buildOverrides) };
}
...
test('the budget line states the ratio, and says so when there is no budget to state', async () => {
  ...
  const noBudget = builtPack({ budgetPct: null });
...
test('an unmeasured token estimate is said out loud, never rendered as an empty pack', async () => {
  ...
  const unmeasured = builtPack({ tokenEstimate: null, budgetPct: null, indexTokenEstimate: null });
...
test('an empty build says so on its built line and in place of its deliveries', async () => {
  ...
  const hollow = pack({ built: build({ empty: true }), hasConsumers: true, deliveredTo: [] });
...
test('measurement formatting reports deliveries and active prompt signals', async () => {
  const { measurementEmptyText, measurementLines } = await importCore();
...
test('outcomes render abort, interruption, and token values', async () => {
  const { outcomeLines } = await importCore();
  assert.deepEqual(outcomeLines({
    outcomes: { sessions: 2, meanInterruptions: 1.5, abortRate: 0.5, meanTokens: 1200 },
  }), [
    {
      label: 'outcomes',
