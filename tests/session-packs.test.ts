...
function fixtureVersion(label: string) {
  return crypto.createHash('sha256').update(label).digest('hex');
}

function fixtureVersionDir(builtRoot: string, name: string, label: string) {
...
test('a session without delivered packs keeps the wakeup matcher alone', async () => {
  ...
  try {
    ...
    assert.deepEqual(settings.hooks.PostToolUse.map((entry: { matcher: string }) => entry.matcher), [WAKEUP_TOOL_MATCHER]);
  } finally {
    session.destroy();
    await fsp.rm(hooksBaseDir, { recursive: true, force: true });
  }
...
test('a configured but unbuilt pack is skipped: no --add-dir, no env flag, a recorded reason', async () => {
  ...
  try {
    ...
    assert.match(String(skips[0].reason), /not built/);
  } finally {
    s.destroy();
    await fsp.rm(builtRoot, { recursive: true, force: true });
  }
...
test('a built dir with no manifest.json is skipped rather than guessed at', async () => {
  ...
  try {
    ...
    assert.deepEqual(calls[0].args, []);
    const skips = s.getDebugState().decisions.filter((d) => d.kind === 'pack');
    assert.match(String(skips[0].reason), /manifest\.json/);
  } finally {
    s.destroy();
    await fsp.rm(builtRoot, { recursive: true, force: true });
  }
...
test('a project with no variant built yet falls back to the base pack, and says so in the trace', async () => {
  ...
  try {
    ...
    assert.match(String(decisions[0].reason), /not built/);
  } finally {
    s.destroy();
    await fsp.rm(builtRoot, { recursive: true, force: true });
  }
...
test('a codex session carries the resolved variant by index pointer without Claude pack flags', async () => {
  ...
  try {
    ...
    const carrierArgs = codex.renderPackArgs([{ name: `memory-${SLUG}`, dir: packDir }], builtRoot) ?? [];
