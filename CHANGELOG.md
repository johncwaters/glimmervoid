...
- **Plan review**: ExitPlanMode requests are captured as retained revisions and open a Plan face on the focused desktop card or phone Terminal screen, with a Plan ready badge, Board attention row, notification routing and `#plan/<id>` deep links. The operator can approve, approve edits, send whole-plan or section feedback, or answer in the terminal; edited markdown returns through Claude Code's own plan write, in-progress drafts are watched read-only, and every revision can be compared with a collapsed line diff. Holds fail open on disconnect, lifecycle changes or timeout, while plan bodies stay behind the authenticated control socket and render through a bounded text-only markdown parser.
- **Agent extensibility**: with `agentApi.enabled` on, a supervised session receives an authenticated `GLIMMERVOID_AGENT_URL` and can run `glimmervoid spawn`, `glimmervoid attention` and `glimmervoid board`. Spawned children are capped, ephemeral siblings with their own worktrees and immediate dashboard cards; early or repeated attention notes are held and combined until the session can raise them. File-only `customAgents` declarations add title-detected CLI agents with configurable command, arguments and busy or idle titles, while Settings lists each declaration and its cached command availability and exposes a danger-confirmed Agent API toggle.
- **GitHub Issues**: an on-demand Issues tab on desktop and phone lists open issues for a configured project and opens a dedicated session with a scrubbed prompt built from the selected issue, without polling or sending issue bodies to the dashboard.
- **Review annotations**: the diff sidebar accepts notes on committed or uncommitted lines, keeps drafts beside their rows and in a notes bar, and sends the batch to the session as one safe bracketed paste for the operator to submit. Each note identifies the path, section, side and correct old or new line number, and drafts survive a deferred paste but expire when their diff section changes.
- **Storage retention**: orphaned uploads, obsolete pack variant directories, missing-transcript tail rows and PostHog scratch files are pruned automatically. Recordings gain the directory-wide `capture.retainBytes` budget, defaulting to 64 MiB when another recording retention bound is active, without removing a file still held open.
- **Environment-provided secrets**: `GLIMMERVOID_POSTHOG_API_KEY` and `GLIMMERVOID_TELEGRAM_BOT_TOKEN` can supply the corresponding config secrets; values supplied this way are stripped from dashboard saves and backups.
- **Flying animals**: animation is now a separate persisted, default-off dashboard preference that works with every theme and scales and repositions its sprites across desktop, phone, resize and orientation changes.
- **Radar and Trace detail**: Radar adds explicit loading states, first and last seen times, issue status, poll interval, live elapsed time, occurrence deltas, titled sparklines and a reachable Open trace action. Trace adds row offsets, turn and tool duration, incremental metrics, kind and error badges, persisted kind filters, expand and collapse controls, and compact subagent identifiers.

### Fixed

- **Terminal grid**: attach now restores a server-maintained terminal screen instead of replaying width-dependent raw history, and the server owns PTY dimensions through ordered claims and size frames so hidden or differently sized viewers cannot corrupt wrapping. Desktop and phone viewers claim only while engaged, retain and retry an unsent claim after reconnect or focus changes, and the phone reclaims its full grid after Android keyboard, background and reopen cycles.
- **Updates**: a release tag already contained by the checkout is no longer offered as an update or refused later as not-fast-forward; the dashboard instead asks for a restart. Failed, interrupted and discarded staging journals are retired at the next boot, while handoff failures and successful handoffs remain visible when operator action may still be required.
- **Memory**: configured retention and per-kind caps now prune the SQLite store and FTS rows instead of only the in-memory view, preserve locked records, run after appends as well as boot, and truncate the WAL. Records from deleted worktrees are re-keyed to their repository when possible, delivered memory hashes are recorded per changed output so the echo filter works, busy writes retry, and delta distillation keeps untouched project claims while advancing through backlog.
- **Detection, trace and Mill metrics**: Claude Code SessionStart now reaches the local command relay so `/clear` and `/compact` reset detection, repeated subagent stops are idempotent, and prompt classification uses the turn boundary rather than the spinner-dominated live state. Trace resume handles a maximum-sized final record and reports a missing subagent transcript once; Mill preserves live accumulators across pack delivery and non-empty outcomes across teardown.
- **Background lanes**: ingest stops retrying deleted worktrees and rolls repeated flush logs into one line per minute, branch cleanup skips only an unusable project instead of backing off every repository, PostHog retries one transient gateway failure, and unrelated settings saves no longer restart secret-bearing PR or PostHog lanes because their config comparisons ignore key order.
- **Session resume**: a Claude Code session whose stored resume id has no matching transcript now starts fresh instead of exiting failed and discarding its clean worktree; Codex and Grok resume behavior is unchanged.
- **Dashboard attention and navigation**: the Visions dot now tracks unacknowledged findings, comments, hands, fixes and intent instead of every background message, the PR and Radar settings links describe their real destinations, the Radar Open trace control no longer disappears inside truncated status text, and help no longer advertises shortcuts that do not work from the resting dashboard.
- **Installation and launchers**: builds mark the declared `glimmervoid` executable runnable for checkout-based installs. Startup and `glimmervoid doctor` now detect a missing `node-pty` native binding before the server loads and print the install-shape-specific repair command required by npm 12.
- **macOS support**: shell ingest defaults to zsh history, Visions editor setup finds VS Code-family app bundles outside `PATH` and accepts verified absolute editor paths, directory comparisons handle symlink and APFS case aliases through real paths, native binding guidance names `xcode-select`, and stop notifications use `osascript` when appropriate.

### Changed

- **Glimmervoid rename**: the product, package, executable, state directory, environment variables, session branch prefix, hook routes, browser identity, editor extension and generated assets are renamed from Glissa to Glimmervoid without compatibility aliases. Configuration now comes only from `GLIMMERVOID_CONFIG` or the Glimmervoid state directory, so an existing installation needs its service repointed and starts with a fresh state directory; old `glissa/session/` branches are outside the new cleanup prefix.
- **Mill measurement**: the pack-read rate and related fields are removed because packs are injected as context rather than opened through the Read tool, eliminating an authenticated PostToolUse request on every Read and removing a metric that could never become non-zero.

## [0.24.3] - 2026-09-07

### Added

- **Session Trace**: every terminal records its Claude Code transcript as normalized records under `~/.glimmervoid/traces` (prompt, skill expansion, thinking, tool call, tool result, assistant) with a checkpoint sidecar so a restart resumes instead of replaying. A Trace tab on the desktop and the phone shows a terminal's trace as turns headed by their prompt, collapsed to one line and expandable, with Load earlier paging back to a 5000-row ceiling. The surfaces follow `debugMode`; capture is on by default behind the file-only `trace.enabled` key.
...
- **The whole project is TypeScript**: every module is strict ESM `.ts`, Vite bundles the server, CLI, relays, and editor extension alongside the dashboard, and the published package ships `dist/` only. The Node floor rises to 22.18. After upgrading, re-run `glimmervoid agent setup grok`: the managed hook entry in `~/.grok/hooks.json` points at the old relay path.
- Provisioned servers that start Glimmervoid with `node server.js` must switch to `node dist/server/index.js` (a source checkout uses `node server/main.ts`); the systemd unit lives in the dotfiles repo.
...
- **Hooks tab**: operator Claude Code hooks are visible in the dashboard alongside the built-in ones Glimmervoid injects.
...
- **First-class Codex CLI sessions**: Glimmervoid now discovers installed agent adapters, offers an agent picker when more than one is available, and reports resolved binaries through `glimmervoid doctor`. Codex sessions gain structural hook and title status detection, safe workspace permissions, resume support, context-pack delivery, per-vendor usage attribution, and an adapter badge.
- **Grok Build sessions**: an opt-in `glimmervoid agent setup grok` command installs a guarded home hook file for supervised Grok sessions, with status detection, resume, background-task completion gating, context-pack delivery, and Grok-specific usage attribution.
- **Opt-in long-term memory**: Glimmervoid can retain HMAC-signed knowledge, intent, feedback, and transcript records in a machine-wide SQLite store with FTS5 retrieval, automatic budgeted backfill across Claude, Codex, and Grok transcripts, retention controls, and secure forgetting that scrubs the row, search index, and WAL. This raises the Node floor to 22.16.0.
...
- **Automatic remote session-branch cleanup**: a default-on lane periodically removes remote `glimmervoid/session/*` branches that are proven merged or orphaned beyond the staleness threshold, while protecting configured sessions and integration branches. Set `branchGc.enabled` to `false` to opt out.
- **Managed rtk support for Codex**: when rtk is enabled but missing, Glimmervoid downloads a pinned release into `~/.glimmervoid/bin`, verifies its pinned SHA-256 digest, reports installation status in Settings and `glimmervoid doctor`, and enables Codex command rewriting through a dedicated guarded hook relay.
- **Native Linux and Pop!_OS support**: Glimmervoid now handles POSIX process groups, worktree links, command resolution, secure config modes, ephemeral files, and session teardown natively, with Linux installation coverage added to CI and platform-specific notification fallback.
...
- **Denser dashboard navigation and reporting**: desktop view tabs now use a centered segmented-pill control, the Mill phone screen uses bounded cards and compact grids, Usage and Mill copy is shorter while preserving provenance labels, and the Glimmervoid server terminal tab identifies itself by port.
- **Leaner bundled Glimmervoid context pack**: the pack now delivers only its distilled brief and rules instead of repeating the raw source documents, reducing its budget from 12,000 to 6,000 tokens.
...
- **External config edits are no longer swallowed**: content-signature suppression now distinguishes Glimmervoid's own writes from later saves, reloads, and operator reversions, closing both timing-window cases that could silently leave disk and live state out of sync.
...
- **Phone image upload**: the phone key strip can upload an image into the active session, saving the file under Glimmervoid's upload area and bracket-pasting the path into the PTY for the operator to send.
...
- **Distribution moved to GitHub installs**: Glimmervoid is no longer published to the npm registry; standalone installs now use `npm install -g github:johncwaters/glimmervoid`, and release/update docs now point at GitHub tags and the `main` branch package metadata.
...
- **Server lifecycle and install diagnostics are quieter and more reliable**: service-managed restarts now exit non-zero under systemd so the supervisor restarts Glimmervoid, pairing writes dedupe stored devices, and PATH diagnostics dedupe duplicate Claude command matches.
...
- **Signals-mode session recording (on by default)**: every session writes a lightweight JSONL forensic log to `~/.glimmervoid/recordings` (header, verbatim hook payloads, state transitions) even when raw PTY capture is off. Raw bytes, input, and resizes stay behind the existing `capture.enabled` opt-in. The file opens lazily on first write, so a session that never starts leaves nothing behind, and retention now caps files per session (`retainFiles`, default 20) alongside the age sweep, both async off the shared event loop. `recordSignals: false` is the kill switch.
...
- **Focus rail keeps a project after its last session closes**: every project path Glimmervoid has seen stays in the rail as an empty group (header, quick-add, dismiss) instead of disappearing and forcing a re-add through the Add Session dialog.
...
- **Crash-safe auto-resume (on by default)**: Glimmervoid captures Claude's session id the moment each conversation starts and persists it immediately, so even a hard kill of Glimmervoid loses nothing. On the next boot, every project that was mid-conversation is respawned with `--resume` and picks up where it left off. Sessions with no captured id stay dormant rather than guessing at a conversation. A new "Auto-Resume" setting (Settings > General, `autoResume` in `config.json`, default on) is the kill switch.
...
- **Startup update check**: on launch Glimmervoid checks the npm registry once for a newer published version and, when one exists, shows a dismissible banner under the header with the running and latest versions and a copy button for the update command (`npm install -g glimmervoid@latest`), plus a one-line console notice. It is advisory and fail-open: any network error, timeout, or offline start is silently ignored and never delays or blocks startup. A new "Check for Updates on Startup" setting (Settings > General, on by default; `checkForUpdates` in `config.json`) is the kill switch, and a dev run from a source checkout is skipped so it never nags while you work on Glimmervoid itself.

## [0.16.0] - 2026-06-19

A maintenance release. It adds install diagnostics (the `glimmervoid doctor` command and post-install PATH guidance), retires the experimental Headroom proxy integration, and stops the burst of Windows console windows that flashed when sessions start and park.

### Added

- **`glimmervoid doctor` diagnostic command**: `glimmervoid doctor` prints a read-only report (glimmervoid and node versions, which install is answering, the npm and pnpm global command directories and whether each is on your PATH, a node-pty load probe, and the resolved config path), so a "glimmervoid is not recognized" or otherwise unhealthy install can be diagnosed in one step. It never starts the server or modifies anything.
- **Post-install PATH guidance**: a global `npm install -g glimmervoid` now prints where the `glimmervoid` command was installed and, when that directory is not on your PATH, the exact one-step fix. The notice is print-only (it never edits your PATH), stays silent for local and dependency installs, and is wrapped so it can never fail an install. A new README "Troubleshooting" section documents the same fix, including pnpm (`pnpm setup`) setups.
...
- **Console windows no longer flash on session start or park**: starting or parking a session could pop a burst of Windows console (`cmd`) windows that stole focus and interrupted work. On a machine where Glimmervoid runs without its own console, the worktree git probes, the junction setup, and the `taskkill` on park each spawned a visible window. Every child process Glimmervoid spawns is now launched hidden, with a guard test that keeps any future spawn site from regressing.
...
- **Menu restart could spawn an unkillable session respawn loop**: Restarting Glimmervoid from the menu and then reopening a session could flood the screen with `cmd` windows in a tight loop that survived closing the Glimmervoid window (only a reboot stopped it). The restart now respawns the replacement hidden (no stray console window) and at most once, waits for the previous session processes to be reaped before exiting so none are orphaned, refuses to start a second instance on the port already in use, and keeps project IDs stable across config reloads so a reload never re-spawns existing sessions.
...
- **Isolated git worktree per session**: Every git-repo session runs in its own worktree forked from the integration branch (`integrationBranch`, default `develop`), so an agent's edits stay out of the main checkout until they are reviewed and merged back. Worktrees default to a `.glimmervoid-worktrees` sibling of the repo, orphans are swept at boot, and every end path (merge, discard, exit) tears the worktree down without losing work.
...
- **Teams: project-level shared pack**: Cross-team pack files (voice-guide, avoid-list, brand) are filled once per project under `.glimmervoid/pack/` and reused by every team that declares them as shared.
- **Deterministic post-turn auto-fix on turn completion**: When a session completes a turn, Glimmervoid runs text-hygiene fixes over its git-changed files (strip em and en dashes and ellipses, trim trailing whitespace, ensure a final newline, strip a UTF-8 BOM) and reports the result on the card; on by default.
...
- **Browser tab favicon**: SVG favicon matching Glimmervoid's purple brand identity.
...
- Hot-reloadable configuration with auto-seeding to `~/.glimmervoid/config.json`
...
[0.16.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.16.0
[0.15.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.15.0
[0.14.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.14.0
[0.13.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.13.0
[0.12.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.12.0
[0.11.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.11.0
[0.10.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.10.0
[0.9.1]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.9.1
[0.9.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.9.0
[0.8.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.8.0
[0.7.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.7.0
[0.6.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.6.0
[0.5.2]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.5.2
[0.5.1]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.5.1
[0.5.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.5.0
[0.4.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.4.0
[0.3.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.3.0
[0.2.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.2.0
[0.1.0]: https://github.com/johncwaters/glimmervoid/releases/tag/v0.1.0
