...
- A pack may NOT be built out of a consumer project's own checkout. Delivery refuses it at spawn (`self-referential`, `server/core/pack-core.ts`): a session already loads those files, and the pack is a lossy copy that drifts silently. Two packs distilled from this repo were retired for exactly that.
...
- The Mill's rows count sessions per PROJECT (its resolved path), never per card record: two records may share one checkout, and a per-record row would list the same project twice (`server/core/pack-core.ts` packConsumerGroups, `tests/pack-core.test.ts`).
...
- A codex card gets packs as ONE `-c developer_instructions` token: a constant directive plus index paths, never `--add-dir` (a writable root), pack bytes, or memory bytes (`session/adapters/codex.ts.renderPackArgs`, `tests/agent-codex.test.ts`).
